            ...
            <div className="card-gold p-8 rounded-3xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {freedomPosts.slice(0, 1).map((p) => (
                  <div key={p.id} className="rounded-2xl overflow-hidden bg-white/5 border border-white/10 md:col-span-2">
                    ...
                    <div className="p-4">
                      ...
                      <div className="mt-3 text-base text-gray-200">{p.content}</div>
                    </div>
                  </div>
                ))}
      ...
      <div className="space-y-6">
        <div className="p-4 rounded-2xl bg-white/5 border border-white/10">
          <textarea
            ...
            className="w-full bg-transparent text-sm text-white focus:outline-none resize-none mb-3"
            rows={3}
          />
          {media && (
            <div className="relative mb-3 rounded-xl overflow-hidden border border-white/10 max-w-xs">
          ...
          <div className="flex justify-between items-center pt-3 border-t border-white/5">
            <div className="flex gap-2">
              <label className="p-2 rounded-lg hover:bg-white/5 text-gray-400 cursor-pointer transition-colors">
                <Plus size={18} />
                <input 
                  type="file" 
                  accept="image/*,video/*" 
          ...
          {posts.slice().reverse().map((post, i) => (
            <div key={i} className="p-4 rounded-2xl bg-white/5 border border-white/5">
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  <div className="text-xs font-bold text-white">{post.sender_name}</div>
          ...
          <div className="max-h-48 overflow-y-auto scrollbar-hide space-y-1">
            {joinedGroups.map(group => {
              ...
              return (
                <button 
                  key={group.id}
                  onClick={() => setActiveRoom(gRoom)}
                  aria-current={activeRoom === gRoom ? 'page' : undefined}
                  className={`w-full flex items-center gap-3 p-3 rounded-xl transition-all ${activeRoom === gRoom ? 'bg-amber-500 text-black' : 'text-gray-400 hover:bg-white/5'} focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-500/30`}
                >
                  <div className="w-5 h-5 rounded bg-amber-500/20 flex items-center justify-center text-[10px] font-bold">
